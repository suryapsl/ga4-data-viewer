WITH top_100_items_by_device AS (
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY device.category ORDER BY COUNT(*) DESC) AS parent_rank
  FROM `ppus-web.analytics_313183936.events_*`,
    UNNEST(items) AS item,
    UNNEST(event_params) AS event_param
  WHERE event_name = 'view_item'
    AND event_param.key = 'page_location'
  GROUP BY device.category, item.item_id
  QUALIFY parent_rank <= 100
),
...
aggregated_select_items AS (
  ...
  LEFT JOIN view_item_images
    ON item.item_id = view_item_images.view_item_id
  WHERE event_name = 'select_item'
    AND SAFE_CAST(item.item_category AS STRING) LIKE '%.html'
  GROUP BY device.category, item.item_id, item.item_category, item.item_category2
),
...
INNER JOIN top_100_items_by_device top_items
  ON top_items.device_category = agg.device_category
  AND top_items.page_location LIKE CONCAT('%', SAFE_CAST(agg.item_category AS STRING), '%')
LEFT JOIN add_to_cart_counts cart
  ON agg.device_category = cart.device_category
  AND agg.clicked_item_id = cart.cart_item_id
  AND agg.item_category = cart.item_category
  AND agg.item_category2 = cart.item_category2
LEFT JOIN purchase_counts purch
  ON agg.device_category = purch.device_category
  AND agg.clicked_item_id = purch.purchase_item_id
  AND agg.item_category = purch.item_category
  AND agg.item_category2 = purch.item_category2
ORDER BY top_items.device_category, top_items.parent_rank, agg.select_item_count DESC;